...
# TODO: It ensures the event loop is unblocked by ensuring no blocking function is called in the same thread of the event loop.
# TODO: The protection is not based on whether the function is called inside an event loop coroutine.
def protect_loop[**_P, _R](
    ...
    # TODO: Wraps the blocking function, so that when they are called inside the event loop, it will raise a RuntimeError.
    @functools.wraps(func)
    def protected_loop_func(*args: _P.args, **kwargs: _P.kwargs) -> _R:
        # TODO: Intended not to only check if the blocking function is called as a callback in the event loop, but raises if it is called in the same thread of the event loop.
