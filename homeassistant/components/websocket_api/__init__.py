...
def async_register_command(
    ...
    if handler is None:
        ...
        # TODO: Extract command and schema from the handler attributes set by the @websocket_command decorator.
        ...
        # TODO: command_or_handler is a command name, use it as KEY in HASS data cache.
        command = command_or_handler
    if (handlers := hass.data.get(DOMAIN)) is None:
        handlers = hass.data[DOMAIN] = {}
    # TODO: Updates hass.data[websocket_api domain] = {
    #     [command]: (handler, schema)
    # }
    handlers[command] = (handler, schema)


# TODO: Invoked in setup.py's async_setup_component() when components/integrations are set up.
# TODO: During integrations set up in `setup.py`, they will invoke this function to register commands to the websocket api singleton instance (singleton as in Python Module Singleton Behavior).
async def async_setup(hass: HomeAssistant, config: ConfigType) -> bool:
    """Initialize the websocket API."""
    # TODO: Register view: The websocketAPI View's route (Get()) is used for establish websocket connection with client, and it is processed and added to the `HomeAssistantApplication`'s router during this phase.
    hass.http.register_view(http.WebsocketAPIView())
    # TODO: Register all the websocket api command handlers with the websocket api, At the end is stored into hass.data[websocket_api domain] = { [command]: (handler, schema) }
